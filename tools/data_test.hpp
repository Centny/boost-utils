...
BOOST_AUTO_TEST_CASE(sub) {
    for (int j = 0; j < 100; j++) {  // test sub
        auto a = butils::tools::BuildData("abc", 3, true);
        a->print();
...
BOOST_AUTO_TEST_CASE(cmp) {
    ...
    auto c = butils::tools::BuildData("abc", 3, true);
    BOOST_CHECK_EQUAL(a->cmp(c.get()), true);
    auto d = butils::tools::BuildData("cde", 3, true);
    BOOST_CHECK_EQUAL(a->cmp(d.get()), false);
}

BOOST_AUTO_TEST_CASE(cmp2) {
    auto a = butils::tools::BuildData("abc", 3, false);
    auto b = a->sub(0, 1, false);
    BOOST_CHECK_EQUAL(a->cmp(b), false);
    auto c = butils::tools::BuildData("abc", 3, false);
    BOOST_CHECK_EQUAL(a->cmp(c.get()), true);
    auto d = butils::tools::BuildData("cde", 3, false);
    BOOST_CHECK_EQUAL(a->cmp(d.get()), false);
    auto e = butils::tools::BuildData(3, false);
    e->len = 0;
    BOOST_CHECK_EQUAL(e->cmp(e), false);
    butils::tools::BuildData(3, true);
}
...
BOOST_AUTO_TEST_CASE(flate) {
    ...
    auto def = butils::tools::BuildData(data, strlen(data), false);
    def->print();
